...
function getDurations(songs) {
    return songs.map((element) => element['duration']);
}

function getDurationInSeconds(songs) {
    return songs.map((element) => element['duration'].split(":")[element['duration'].split(":").length-1]);
}

function getMainArtists(songs) {
    return songs.map((element) => element['name'].split('featuring')[0]);
