...
function getBigHits(songs) {
    return songs.filter((item) => item['weeksAtNumberOne'] >= 10);

}

function getShortSongs(songs) {
    return songs.filter((element) => parseInt(element['duration'][0]) < 3 && parseInt(element['duration'].split(':')[element['duration'].split(':').length-1]));
}


function getSongsByArtist(songs, artist) {
    
    return songs.filter((element) => element.artist.match(artist));
}

// console.log( getSongsByArtist(songs, 'Adele') );
